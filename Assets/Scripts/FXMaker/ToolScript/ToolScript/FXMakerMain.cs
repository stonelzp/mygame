...
[AddComponentMenu("FXMaker/ToolScript/FXMakerMain")]
public class FXMakerMain : MonoBehaviour
{
	...
	// directroy
	public		string					m_IGSoft_ResourcesDir		= "Assets";
	public		enum TOOLDIR_TYPE		{DEFAULTSHADERMATERIALS=0, TEMPPREVIEWTEXTURE, BACKGROUNDPREFABS, BACKGROUNDRESOURCES, PROJECTS, SCRIPTS, SCREENSHOTSAVEDIR, CURVE_INFO, SPRITE_TOOL, SPRITE_USER};
	protected	string					m_DefaultShaderMaterialsDir	= "Standard Assets/FXMaker/ToolResources/DefaultShaderMaterials";
	protected	string					m_TempPreviewTextureDir		= "Standard Assets/FXMaker/ToolResources/TempPreviewTexture";
	protected	string					m_BackgroundPrefabsDir		= "Standard Assets/FXMaker/Backgrounds/BackgroundPrefabs";
	protected	string					m_BackgroundResourcesDir	= "Standard Assets/FXMaker/Backgrounds/BackgroundResources";
	protected	string					m_ProjectsDir				= "Effect";
	...
	public string GetResourceDir(TOOLDIR_TYPE tooldirType)
	{
		...
		switch (tooldirType)
		{
			...
			case TOOLDIR_TYPE.CURVE_INFO:				subDir = "Effect/[Resources]/[Curve]";				break;
			case TOOLDIR_TYPE.SPRITE_TOOL:				subDir = "Effect/[Resources]/[Sprite]/ToolSprite";	break;
			case TOOLDIR_TYPE.SPRITE_USER:				subDir = "Effect/[Resources]/[Sprite]/USerSprite";	break;
...
//		FXMakerHierarchy.inst.ChangeBoundsBoxWireframe(indexCom.m_OriginalTrans.gameObject, null, bRoot, true);
	...
	public GameObject ClearCurrentEffectObject(GameObject effectRoot, bool bClearEventObject)
	{
		...
		if (bClearEventObject)
		{
			...
			if (parentObj != null)
				NgObject.RemoveAllChildObject(parentObj);
		}

		NgObject.RemoveAllChildObject(effectRoot);
	...
	public void SetCurrentEffectPrefab(GameObject setPrefab, GameObject effectRoot, bool bUpdateOnlyPrefab)
	{
		...
		if(null == m_OriginalEffectPrefab.GetComponent<NcAutoDestruct>())
		{
			NcAutoDestruct des = m_OriginalEffectPrefab.AddComponent<NcAutoDestruct>();
			des.m_fLifeTime = -1;
			des.enabled = false;
		}
		...
		m_OriginalEffectObject.SetActive(false);

		
		// 解决遗留问题(不在需要NcDestroyEvent去控制特效的销毁)
		foreach(NcDestroyEvent evt in m_OriginalEffectObject.GetComponents<NcDestroyEvent>())
		{
			Destroy(evt);
			FXMakerEffect.inst.SetChangePrefab();
		}
...
#pragma warning disable
			createObj.SetActiveRecursively(true);
...
// 				OnHoverCommand_Hierarchy(0);
	...
	void UnactiveOriginalObject()
	{
		...
		if (effectToolMain != null && effectToolMain.m_CurrentEffectRoot != null && GetOriginalEffectObject() != null)
		{
			...
			// Unactive
			GetOriginalEffectObject().SetActive(false);
