...
[AddComponentMenu("FXMaker/ToolScript/FXMakerGizmo")]
...
// 		FXMakerMain.inst.GetFXMakerMouse().SetHandControl(((GIZMO_TYPE)m_nGizmoTypeIndex) == GIZMO_TYPE.HAND);
	...
	void UpdateGrid(bool bShow)
	{
		...
		{
			m_GridXYGameObject.SetActive(bShow && m_bGridXY);
		...
		{
			m_GridXZGameObject.SetActive(bShow && m_bGridXZ);
			m_GridXYGameObject.transform.localScale	= texSize;
			m_GridXYGameObject.GetComponent<Renderer>().sharedMaterial.mainTextureScale	= texUnit;
		}
		if (m_GridYZGameObject != null)
		{
			m_GridYZGameObject.SetActive(bShow && m_bGridYZ);
			m_GridXYGameObject.transform.localScale	= texSize;
			m_GridXYGameObject.GetComponent<Renderer>().sharedMaterial.mainTextureScale	= texUnit;
		}
	}
	...
	{
		GUIContent[]	cons = FXMakerTooltip.GetHcGizmo();

		cons[0].image	= (m_nGizmoTypeIndex == 0) ? m_iconHandActive	: m_iconHandNormal;
		cons[1].image	= (m_nGizmoTypeIndex == 1) ? m_iconPosActive	: m_iconPosNormal;
		cons[2].image	= (m_nGizmoTypeIndex == 2) ? m_iconRotActive	: m_iconRotNormal;
		cons[3].image	= (m_nGizmoTypeIndex == 3) ? m_iconScaleActive	: m_iconScaleNormal;
		cons[4].image	= (m_nGizmoTypeIndex == 4) ? m_iconNoneActive	: m_iconNoneNormal;
		return cons;
	}
}
