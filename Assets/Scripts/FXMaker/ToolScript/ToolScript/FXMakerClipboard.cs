...
[AddComponentMenu("FXMaker/ToolScript/FXMakerClipboard")]
...
//			NgMaterial.SetMaterialColor(tarMat, m_CopyColor);
	...
	public Object OverwriteClipboardObject(GameObject tarGameObject, Object tarComponent, int tarIndex)
	{
		if (IsGameObject())
		{
			...
			newObj.transform.parent = tarGameObject.transform; // 坷幅乐促... 荤侩救窍骨肺 唱吝俊 贸府
	...
	void SetClipboardTransform(Transform selTrans)
	{
		...
		(m_CopyCloneObject as Transform).gameObject.SetActive(false);
	...
	void SetClipboardComponent(Component selCom)
	{
		...
		m_CopyCloneGameObject.SetActive(false);
	...
	public static Component ObjectCopy(Component src, Transform dstSelTrans, Object dstSelObj, bool bCreateNewTarget, bool bSearchTarComponent, bool bCopyAllTarComs)
	{
		...
		// bCreateNewTarget 曼捞咯档, bSearchTarComponent 曼捞搁 货肺 积己救窃

		if ((bCreateNewTarget == false) && src.GetType() == dstSelObj.GetType())	// 脚痹积己 炼扒捞 酒聪哥, 鞍篮霸俊 靛而吝
		{
// 			if (NgAssembly.IsValidCopy(dst, src))
		...
		if (tarComs == null || tarComs.Length <= 0)					// 绝促 父甸磊
		{
			tarComs		= new Component[1];
		...
		if (tarComs != null)
		{
			foreach (Component tarcom in tarComs)					// 郴侩 汗荤
			{
				if (src != tarcom)
				{
