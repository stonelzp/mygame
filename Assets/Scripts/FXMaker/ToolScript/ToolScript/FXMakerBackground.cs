...
[AddComponentMenu("FXMaker/ToolScript/FXMakerBackground")]
public class FXMakerBackground : MonoBehaviour
{
	...
	public			GameObject			m_DefaultBackgroundPrefab;		// 积己侩 叼弃飘 prefab
...
// 			m_TestCubeObject.activeSelf = false;
// 		if (m_TestSphereObject != null)
// 			m_TestSphereObject.activeSelf = false;
	}
	...
	// group客 prefab path肺 group 棺 Background prefab index甫 瘤沥茄促.
...
// 			Debug.Log(loaddir);
			m_BackgroundPrefabs	= NgAsset.GetPrefabList(loaddir, true, true, 0, out m_nBackgroundCount);
	...
	void LoadResourceFolder()
	{
		for (int n = 0; n < m_CloneTypeCount; n++)
		{
			...
			m_ClonePrefabs[n]		= NgAsset.GetPrefabList(strDir, true, true, FXMakerLayout.m_nMaxResourceListCount, out nFileCount);
	...
	{
		// clear current
		NgObject.RemoveAllChildObject(m_CurrentBackgroundRoot);
		if (m_CurrentBackgroundInfo != null)
