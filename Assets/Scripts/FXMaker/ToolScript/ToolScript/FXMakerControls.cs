...
[AddComponentMenu("FXMaker/ToolScript/FXMakerControls")]
...
//		m_fStartPosition	= fStartAdjustRate;
		...
		if (FXMakerMain.inst.GetInstanceEffectObject() == null && IsAutoRepeat() == false)
		{
			...
			if (m_fDelayCreateTime < Time.time - m_fPlayStartTime)
			{
				...
				if (m_nTransIndex == 0 && IsAutoRepeat() && m_bCalledDelayCreate == false)	// m_bStartAliveAnimation
				{
					...
					{
						GameObject go = FXMakerMain.inst.GetOriginalEffectObject();
						if(null != go)
						{
							NcAutoDestruct des = go.GetComponent<NcAutoDestruct>();
							if(null != des && des.m_bAutoSetLifeTime)
							{
								des.m_fLifeTime = Time.time - m_fPlayStartTime;
	...
	bool IsAliveAnimation()
	{
		...
		foreach (Transform trans in tranComs)
		{
			...
			// Check Animation
			if(trans.GetComponent<Animation>() && trans.GetComponent<Animation>().enabled && trans.GetComponent<Animation>().isPlaying)
				return true;
			...
			{
				bParticle = 0;
				if (trans.gameObject.activeSelf && ((trans.GetComponent<ParticleSystem>().enableEmission && trans.GetComponent<ParticleSystem>().IsAlive()) || 0 < trans.GetComponent<ParticleSystem>().particleCount))
					bParticle = 1;
			}
			...
			if (bParticle < 1)
			{
				...
				{
					bParticle = 0;
					if (trans.gameObject.activeSelf && (trans.GetComponent<ParticleEmitter>().emit || 0 < trans.GetComponent<ParticleEmitter>().particleCount))
						bParticle = 1;
				}
			}
...
/*			if (trans.renderer != null)
			{
				if (trans.GetComponent<FXMakerWireframe>() == null && trans.renderer.enabled && trans.gameObject.activeSelf)
					bRen = true;
			}
...
// 			TransSpeed = 1;
	...
	protected void RunActionControl(int nPlayIndex, int nTransIndex)
	{
		...
		// Á€ÁöµÈ œÃ°èÀÏ °æ¿ì ¿òÁ÷ÀÌµµ·Ï ÇØµÎÀÚ...
		ResumeTimeScale();
		...
		switch (nPlayIndex)
		{
			...
			case m_nRepeatIndex+4: {
						// repeatŒ±ÅÃÀÌžé.. ÀÌµ¿°ü·Ã ¿ÉŒÇÀ» ²öŽÙ.
...
// 				m_nMultiShotCount = 1;
		...
		// ÀÌµ¿Ã³ž®ÁßÀÌžé... ÀÚµ¿¹Ýº¹µÇ¹Ç·Î ±âÁžrepeat¿ÉŒÇÀº ²öŽÙ.
		if (0 < nTransIndex)
...
//		Debug.Log("CreateInstanceEffect");
	...
	public void SetTimeScale(float timeScale)
	{
		if (m_fTimeScale != timeScale || m_fTimeScale != Time.timeScale)
		{
			...
			if (0.01f <= m_fTimeScale)	// ŽÙœÃ œÃÀÛÇßÀ»¶§ žØÃáÇö»óÀž·Î Âø°¢ŸÈÇÏµµ·Ï..
