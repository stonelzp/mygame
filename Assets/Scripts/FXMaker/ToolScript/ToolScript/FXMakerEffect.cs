...
[AddComponentMenu("FXMaker/ToolScript/FXMakerEffect")]
...
// 		Debug.Log(defaultEffectPath);
	...
	int LoadEffectFolder(string selectPrefabPath)
	{
		...
		if (0 < m_nGroupCount)
		{
			...
			m_EffectPrefabs		= NgAsset.GetPrefabList(m_LoadDirectory, true, true, FXMakerLayout.m_nMaxPrefabListCount, out m_nEffectCount);
...
//		FXMakerMain.inst.ToggleGlobalLangSkin(false);
		...
		if (bChanged)
		{
			...
			SaveProject();
			if(nProjectIndex < m_nProjectCount)
