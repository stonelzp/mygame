...
//		return (int)FXMakerOption.inst.m_LanguageType;
	...
	public static string GetHsToolMessage(string keyword, string arg)
	{
		...
		{
			case "": break;
			case "FOLDER_READONLY"			: tooltip = helpStr[GetLang(), 0];		break;
			case "FOLDER_READONLY_BOTTOM"	: tooltip = helpStr[GetLang(), 1];		break;
			case "MATERIAL_NEWSAVED"		: tooltip = helpStr[GetLang(), 2];		break;
			case "MATERIAL_EXISTSAVED"		: tooltip = helpStr[GetLang(), 3];		break;
			case "DIALOG_DELETEPREFAB"		: tooltip = helpStr[GetLang(), 4];		break;
			case "DIALOG_DELETESPRITE"		: tooltip = helpStr[GetLang(), 5];		break;
			case "WARRING_AUTOSAVE"			: tooltip = helpStr[GetLang(), 6];		break;
			case "TOOL_AUTOSAVE"			: tooltip = helpStr[GetLang(), 7];		break;
			case "READONLY_FOLDER"			: tooltip = helpStr[GetLang(), 8];		break;
			case "READONLY_NOTSAVE"			: tooltip = helpStr[GetLang(), 9];		break;
	...
	public static string GetHsScriptMessage(string keyword)
	{
		...
		{
			case "": break;
			case "SCRIPT_EMPTY_CURVE"					: tooltip = helpStr[GetLang(), 0];		break;
			case "SCRIPT_EMPTY_ATTACHPREFAB"			: tooltip = helpStr[GetLang(), 1];		break;
			case "SCRIPT_EMPTY_AUDIOCLIP"				: tooltip = helpStr[GetLang(), 2];		break;
			case "SCRIPT_ERROR_ROOT"					: tooltip = helpStr[GetLang(), 3];		break;
			case "SCRIPT_EMPTY_PARTICLE"				: tooltip = helpStr[GetLang(), 4];		break;

			case "SCRIPT_NEED_SCALEPARTICLE"			: tooltip = helpStr[GetLang(), 5];		break;
			case "SCRIPT_WARRING_SCALESPEED"			: tooltip = helpStr[GetLang(), 6];		break;
			case "SCRIPT_WARRING_SCALESPEED_SHURIKEN"	: tooltip = helpStr[GetLang(), 7];		break;
			case "SCRIPT_NEED_NCUVANIMATION"			: tooltip = helpStr[GetLang(), 8];		break;
			case "SCRIPT_WARRING_NCDETACHPARENT"		: tooltip = helpStr[GetLang(), 9];		break;

			case "SCRIPT_WARRING_DUPLICATE"				: tooltip = helpStr[GetLang(),10];		break;
			case "SCRIPT_EMPTY_COLOR"					: tooltip = helpStr[GetLang(),11];		break;

			case "SCRIPT_EMPTY_MATERIAL"				: tooltip = helpStr[GetLang(),12];		break;
			case "SCRIPT_DUPERR_EDITINGUV"				: tooltip = helpStr[GetLang(),13];		break;
			case "SCRIPT_CLASH_ROTATEBILL"				: tooltip = helpStr[GetLang(),14];		break;
			case "SCRIPT_EMPTY_SPRITEFACTORY"			: tooltip = helpStr[GetLang(),15];		break;

			case "SCRIPT_NEED_RIGIDBODY"				: tooltip = helpStr[GetLang(),16];		break;
	...
	public static GUIContent GetHcToolMain(string caption, string arg)
	{
		...
		{
			case "": break;
			case "go Background"	: tooltip = helpStr[GetLang(), 0];		break;
			case "go PrefabTool"	: tooltip = helpStr[GetLang(), 1];		break;
			case "LoadPrj"			: tooltip = helpStr[GetLang(), 2];		break;
			case "SavePrj"			: tooltip = helpStr[GetLang(), 3];		break;
			case "FullCapture"		: tooltip = helpStr[GetLang(), 4] + "\n" + arg;		break;
...
// 			case "BACKGROUND_HOVER"		: tooltip = arg + "\n" + helpStr[GetLang(), 3]		+ AddHintRect(FXMakerBackground.GetRect_Resources); break;
	...
	public static GUIContent GetHcToolEffect(string caption)
	{
		...
		{
			case ""			: break;
			case "New"		: tooltip	= helpStr[GetLang(), 0];	break;
			case "Del"		: tooltip	= helpStr[GetLang(), 1]		+ FXMakerOption.inst.m_bActiveRecycleBin.ToString();	break;
			case "Clone"	: tooltip	= helpStr[GetLang(), 2];	break;
			case "Thumb"	: tooltip	= helpStr[GetLang(), 3]		+ FXMakerTooltip.AddCommand("ShowThumbCaptureRect");	break;
			case "History"	: tooltip	= helpStr[GetLang(), 4];	break;
			case "Sprite"	: tooltip	= helpStr[GetLang(), 5]		+ FXMakerTooltip.AddCommand("ShowSpriteCaptureRect");	break;
			default: TooltipError(caption); break;
		}
		if (tooltip != null) tooltip = caption + "\n" + tooltip;
		return GetGUIContent(caption, tooltip);
	...
	{
		GUIContent[]	cons = new GUIContent[4];

		cons[0]	= GetGUIContent("Top"	, "Top View" + AddHintRect(FXMakerLayout.GetClientRect));
		cons[1]	= GetGUIContent("HighQ"	, "High Quarter View (90)" + AddHintRect(FXMakerLayout.GetClientRect));
		cons[2]	= GetGUIContent("Back"	, "Back View" + AddHintRect(FXMakerLayout.GetClientRect));
		cons[3]	= GetGUIContent("LowQ"	, "Low Quarter View (40)" + AddHintRect(FXMakerLayout.GetClientRect));
		return cons;
	}
	...
	public static GUIContent GetHcEffectControls(string caption, string arg)
	{
		...
		switch (caption)
		{
			...
			case "Play"			:	tooltip = helpStr[GetLang(), 0];		break;
			case "Trans"		:	tooltip = helpStr[GetLang(), 1];		break;
			case "DistPerTime"	:	tooltip = helpStr[GetLang(), 2];		break;
			case "TimeScale"	:	tooltip = helpStr[GetLang(), 3];		break;
			case "ElapsedTime"	:	tooltip = helpStr[GetLang(), 4];		break;

			case "1"			:	tooltip = helpStr[GetLang(), 5];		break;
			case "<"			:	tooltip = helpStr[GetLang(), 6];		break;
			case ">"			:	tooltip = helpStr[GetLang(), 7];		break;
			case "Multi"		:	return GetGUIContent(arg + " " + caption, helpStr[GetLang(), 8]		+ AddHintRect(FXMakerLayout.GetClientRect));

			case "Reset"		:	tooltip = helpStr[GetLang(), 9];		break;
			case "Pause"		:	tooltip = helpStr[GetLang(), 10];		break;
			case "Resume"		:	tooltip = helpStr[GetLang(), 11];		break;

			case "Restart"		:	tooltip = helpStr[GetLang(), 12];		break;
			case "+.5"			:	tooltip = helpStr[GetLang(), 13];		break;
			case "+.1"			:	tooltip = helpStr[GetLang(), 14];		break;
			case "+.05"			:	tooltip = helpStr[GetLang(), 15];		break;
			case "+.01"			:	tooltip = helpStr[GetLang(), 16];		break;
	...
	public static GUIContent[] GetHcEffectControls_Play(float fAutoRet, float timeScale, float timeOneShot1, float timeRepeat1, float timeRepeat2, float timeRepeat3, float timeRepeat4, float timeRepeat5)
	{
		...
		cons[0]	= GetGUIContent("AutoRet"	, helpStr[GetLang(), 0] + AddHintRect(FXMakerLayout.GetClientRect));
		cons[1]	= GetGUIContent(timeScale.ToString("0.00")+"x S"	, helpStr[GetLang(), 1]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[2]	= GetGUIContent(timeOneShot1.ToString("0.0")+"x S"	, "- " + string.Format(helpStr[GetLang(), 2], timeOneShot1.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[3]	= GetGUIContent(timeRepeat1.ToString("0.0")+"s R"	, "- " + string.Format(helpStr[GetLang(), 3], timeRepeat1.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[4]	= GetGUIContent(timeRepeat2.ToString("0.0")+"s R"	, "- " + string.Format(helpStr[GetLang(), 3], timeRepeat2.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[5]	= GetGUIContent(timeRepeat3.ToString("0.0")+"s R"	, "- " + string.Format(helpStr[GetLang(), 3], timeRepeat3.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[6]	= GetGUIContent(timeRepeat4.ToString("0.0")+"s R"	, "- " + string.Format(helpStr[GetLang(), 3], timeRepeat4.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[7]	= GetGUIContent(timeRepeat5.ToString("0.0")+"s R"	, "- " + string.Format(helpStr[GetLang(), 3], timeRepeat5.ToString("0.0"))	+ AddHintRect(FXMakerLayout.GetClientRect));
	...
	public static GUIContent[] GetHcEffectControls_Trans(NgEnum.AXIS nTransAxis)
	{
		...
		cons[0]	= GetGUIContent("Stop"		, helpStr[GetLang(), 0] + AddHintRect(FXMakerLayout.GetClientRect));
		cons[1]	= GetGUIContent(nTransAxis.ToString() + " Move"	, "- " + string.Format(helpStr[GetLang(), 1], nTransAxis.ToString()) + AddHintRect(FXMakerLayout.GetClientRect));
		cons[2]	= GetGUIContent(nTransAxis.ToString() + " Scale"	, "- " + string.Format(helpStr[GetLang(), 2], nTransAxis.ToString())+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[3]	= GetGUIContent("Arc"		, helpStr[GetLang(), 3]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[4]	= GetGUIContent("Fall"		, helpStr[GetLang(), 4]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[5]	= GetGUIContent("Raise"		, helpStr[GetLang(), 5]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[6]	= GetGUIContent("Circle"	, helpStr[GetLang(), 6]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[7]	= GetGUIContent("Tornado"	, helpStr[GetLang(), 7]	+ AddHintRect(FXMakerLayout.GetClientRect));
		return cons;
	}
	...
	public static GUIContent[] GetHcEffectControls_Rotate()
	{
		...
		cons[0]	= GetGUIContent("Rot"	, helpStr[GetLang(), 0]	+ AddHintRect(FXMakerLayout.GetClientRect));
		cons[1]	= GetGUIContent("Fix"	, helpStr[GetLang(), 1]	+ AddHintRect(FXMakerLayout.GetClientRect));
		return cons;
	}
	...
	public static GUIContent[] GetHcGizmo()
	{
		...
		CheckValid(helpStr);
		GUIContent[]	cons = new GUIContent[5];

		cons[0]	= GetGUIContentImage(null, "Hand [ ShotKey = Q ]\n"		+ helpStr[GetLang(), 0]);
		cons[1]	= GetGUIContentImage(null, "Position [ ShotKey = W ]\n" + helpStr[GetLang(), 1]);
		cons[2]	= GetGUIContentImage(null, "Rotation [ ShotKey = E ]\n" + helpStr[GetLang(), 2]);
		cons[3]	= GetGUIContentImage(null, "Scale [ ShotKey = R ]\n"	+ helpStr[GetLang(), 3]);
		cons[4]	= GetGUIContentImage(null, "None [ ShotKey = T ]\n"		+ helpStr[GetLang(), 4]);
		return cons;
	}
	...
	public static GUIContent GetHcEffectHierarchy(string caption)
	{
		...
		switch (caption)
		{
			...
			case "Popup_GameObject"		:	caption = "E";		tooltip = helpStr[GetLang(), 9];	break;
			case "Popup_AddPrefab"		:	caption = "P";		tooltip = "- Add Prefab"	+ "\n" + helpStr[GetLang(), 10]; break;
			case "Popup_Mesh"			:	caption = "...";	tooltip = "- Change Mesh"	+ "\n" + helpStr[GetLang(), 11]; break;
			case "Popup_Transform"		:	caption = "...";	tooltip = "- Edit Transform";	break;
			case "Popup_Material"		:	caption = "...";	tooltip = helpStr[GetLang(), 13]; break;
			case "AddOn_AddMaterial"	:	caption = "+";		tooltip = helpStr[GetLang(), 14]; break;
...
//		CheckValid(helpStr);
	...
	public static GUIContent GetHcPopup_Sprite(string caption)
	{
		...
		{
			case "": break;
			case "CaptureType":			tooltip	= helpStr[GetLang(), 1]; break;
			case "Size":				tooltip	= helpStr[GetLang(), 2]; break;
			case "Fps":					tooltip	= helpStr[GetLang(), 3]; break;
			case "Time":				tooltip = helpStr[GetLang(), 4]; break;
			case "SkipFrame":			tooltip = helpStr[GetLang(), 5]; break;
			case "PlayMode":			tooltip = helpStr[GetLang(), 6]; break;
			case "TotalFrame":			tooltip = helpStr[GetLang(), 7]; break;
			case "TextureSize":			tooltip = helpStr[GetLang(), 8]; break;
			case "Shader":				tooltip = helpStr[GetLang(), 9]; break;
			case "Fade":				tooltip = helpStr[GetLang(),10]; break;
			case "Create":				tooltip = helpStr[GetLang(),11]; break;
			case "TextureType":			tooltip = helpStr[GetLang(),12]; break;
			case "MaxSize":				tooltip = helpStr[GetLang(),13]; break;
			case "Format":				tooltip = helpStr[GetLang(),14]; break;
			case "Loop":				tooltip = helpStr[GetLang(),15]; break;
			case "AnisoLevel":			tooltip = helpStr[GetLang(),16]; break;
			case "CaptureCount":		tooltip = helpStr[GetLang(),17]; break;
			case "SkipTime":			tooltip = helpStr[GetLang(),18]; break;
	...
	public static GUIContent GetHcPopup_GameObject(string caption, string arg)
	{
		...
		switch (caption)
		{
			case "": break;
			// GameObject
			case "Copy":			tooltip	= helpStr[GetLang(), 0];	break;
			case "Cut": 			tooltip	= helpStr[GetLang(), 1];	break;
			case "Paste": 			tooltip	= helpStr[GetLang(), 2]		+ "\n" + "Source : " + arg; break;
			case "Overwrite":		tooltip	= helpStr[GetLang(), 3]		+ "\n" + "Source : " + arg; break;
			case "Duplicate": 		tooltip	= "- Duplicate" + arg; break;
			case "Enable":			tooltip	= helpStr[GetLang(), 5];	break;
			case "Disable":			tooltip	= helpStr[GetLang(), 6];	break;
			case "Delete": 			tooltip	= helpStr[GetLang(), 7];	break;

			case "Add Child": 		tooltip	= helpStr[GetLang(), 8];	break;
			case "Add Prefab": 		tooltip	= helpStr[GetLang(), 9];	break;
			case "Add Parent": 		tooltip	= helpStr[GetLang(), 10];	break;
			case "MoveToParent": 	tooltip	= helpStr[GetLang(), 11];	break;
			case "Save Prefab": 	tooltip	= helpStr[GetLang(), 12];	break;
			case "Copy Color":	 	tooltip	= helpStr[GetLang(), 13];	break;
			case "Paste Color":		tooltip	= helpStr[GetLang(), 14];	break;

			case "Add Component": 	tooltip	= helpStr[GetLang(), 15];	break;
			case "Add Component ": 	tooltip	= helpStr[GetLang(), 16];	break;


			// Component
			case "SaveCurves": 		tooltip	= helpStr[GetLang(), 17];	break;
			case "LoadCurves": 		tooltip	= helpStr[GetLang(), 18];	break;
	...
	public static GUIContent GetHcFolderPopup_Common(string caption)
	{
		...
		{
			case "": break;
			case "ShowName":		tooltip	= helpStr[GetLang(), 0]; break;
			case "Recursively": 	tooltip	= helpStr[GetLang(), 1]; break;
			case "Close":			tooltip = helpStr[GetLang(), 2]; break;
			case "Cancel":			tooltip = helpStr[GetLang(), 3]; break;
			case "Save":			tooltip = helpStr[GetLang(), 4]; break;
			case "Undo":			tooltip = helpStr[GetLang(), 5]; break;
			default: TooltipError(caption); break;
		}
		return GetGUIContent(caption, tooltip);
	}
	...
	public static GUIContent GetHcFolderPopup_NcInfoCurve(string caption)
	{
		...
		{
			case "": break;
			case "Overwrite":		tooltip	= helpStr[GetLang(), 0]; break;
			case "Save":			tooltip	= helpStr[GetLang(), 1]; break;
	...
	public static string GetHsFolderPopup_Texture(string caption)
	{
		...
		switch (caption)
		{
			case "HOVER_DESC":	return helpStr[GetLang(), 0];
	...
	public static string GetHsEditor_FXMakerOption(string caption)
	{
		...
		switch (caption)
		{
			case "m_bHintRedBox":				tooltip	= helpStr[GetLang(), 1];	break;
			case "m_bShowCursorTooltip":		tooltip	= helpStr[GetLang(), 2];	break;
			case "m_bShowBottomTooltip":		tooltip	= helpStr[GetLang(), 3];	break;
			case "m_LanguageType":				tooltip	= helpStr[GetLang(), 4];	break;
			case "m_nMinTopToolbarCount":		tooltip	= helpStr[GetLang(), 5];	break;
			case "m_nMinBottomToolbarCount":	tooltip	= helpStr[GetLang(), 6];	break;
			case "m_fFixedWindowWidth":			tooltip	= helpStr[GetLang(), 7];	break;
			case "m_fTopMenuHeight":			tooltip	= helpStr[GetLang(), 8];	break;
			case "m_bActiveRecycleBin":			tooltip	= helpStr[GetLang(), 9];	break;
			case "m_bAutoSaveAppPause":			tooltip	= helpStr[GetLang(), 10];	break;
			case "m_bAutoSaveAppQuit":			tooltip	= helpStr[GetLang(), 11];	break;
			case "m_fScaleTransSpeed":			tooltip	= helpStr[GetLang(), 12];	break;
			case "m_SimulateArcCurve":			tooltip	= helpStr[GetLang(), 13];	break;
			case "m_bDragDrop_WorldSpace":		tooltip	= helpStr[GetLang(), 14];	break;
			case "m_ColorRootBoundsBox":		tooltip	= helpStr[GetLang(), 15];	break;
			case "m_ColorChildBoundsBox":		tooltip	= helpStr[GetLang(), 16];	break;
			case "m_ColorRootWireframe":		tooltip	= helpStr[GetLang(), 17];	break;
			case "m_ColorChildWireframe":		tooltip	= helpStr[GetLang(), 18];	break;

			case "m_AddPrefabPosition":			tooltip	= helpStr[GetLang(), 19];	break;
			case "m_PrefabDlg_RightClick":		tooltip	= helpStr[GetLang(), 20];	break;

			case "m_GridUnit":					tooltip	= helpStr[GetLang(), 21];	break;
			case "m_GridSize":					tooltip	= helpStr[GetLang(), 22];	break;
			case "m_bUpdateNewMaterial":		tooltip	= helpStr[GetLang(), 23];	break;
	...
	public static string GetHsEditor_NcCurveAnimation(string caption)
	{
		...
		switch (caption)
		{
			case "m_fDelayTime":			tooltip	= helpStr[GetLang(), 1];	break;
			case "m_fDurationTime":			tooltip	= helpStr[GetLang(), 2];	break;
			case "m_bAutoDestruct":			tooltip	= helpStr[GetLang(), 3];	break;

			case "Clear All":				tooltip	= helpStr[GetLang(), 4];	break;
			case "Load Curve":				tooltip	= helpStr[GetLang(), 5];	break;
			case "Save Curve":				tooltip	= helpStr[GetLang(), 6];	break;
			case "Add EmptyCurve":			tooltip	= helpStr[GetLang(), 7];	break;

			case "Add NcUvAnimation Script":tooltip	= helpStr[GetLang(), 8];	break;
			case "White":					tooltip	= helpStr[GetLang(), 9];	break;
			case "Current":					tooltip	= helpStr[GetLang(), 10];	break;
			case "ToColor":					tooltip	= helpStr[GetLang(), 11];	break;
			case "Recursively":				tooltip	= helpStr[GetLang(), 12];	break;
			case "Value Scale":				tooltip	= helpStr[GetLang(), 13];	break;

			case "Change":					tooltip	= helpStr[GetLang(), 14];	break;
			case "Save":					tooltip	= helpStr[GetLang(), 15];	break;
			case "Delete":					tooltip	= helpStr[GetLang(), 16];	break;
	...
	public static string GetHsEditor_NcSpriteAnimation(string caption)
	{
		...
		switch (caption)
		{
			case "m_PlayMode":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fDelayTime":		tooltip	= helpStr[GetLang(), 2];		break;
			case "m_nStartFrame":		tooltip	= helpStr[GetLang(), 3];		break;
			case "m_nFrameCount":		tooltip	= helpStr[GetLang(), 4];		break;
			case "m_nSelectFrame":		tooltip	= helpStr[GetLang(), 5];		break;
			case "m_bLoop":				tooltip	= helpStr[GetLang(), 6];		break;
			case "m_bAutoDestruct":		tooltip	= helpStr[GetLang(), 7];		break;
			case "m_fFps":				tooltip	= helpStr[GetLang(), 8];		break;
			case "m_nTilingX":			tooltip	= helpStr[GetLang(), 9];		break;
			case "m_nTilingY":			tooltip	= helpStr[GetLang(),10];		break;
			case "DurationTime":		tooltip	= helpStr[GetLang(),11];		break;
	...
	public static string GetHsEditor_NcAddForce(string caption)
	{
		...
		switch (caption)
		{
			case "m_AddForce":				tooltip	= helpStr[GetLang(), 1];		break;
			case "m_RandomRange":			tooltip	= helpStr[GetLang(), 2];		break;
			case "m_ForceMode":				tooltip	= helpStr[GetLang(), 3];		break;

			case "Add RigidBody Component":	tooltip	= helpStr[GetLang(), 4];		break;
	...
	public static string GetHsEditor_NcAttachPrefab(string caption)
	{
		...
		switch (caption)
		{
			case "m_AttachType":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fDelayTime":			tooltip	= helpStr[GetLang(), 2];		break;
			case "m_fRepeatTime":			tooltip	= helpStr[GetLang(), 3];		break;
			case "m_nRepeatCount":			tooltip	= helpStr[GetLang(), 4];		break;

			case "m_AttachPrefab":			tooltip	= helpStr[GetLang(), 5];		break;
			case "m_bWorldSpace":			tooltip	= helpStr[GetLang(), 6];		break;
			case "m_AccumStartRot":			tooltip	= helpStr[GetLang(), 7];		break;
			case "m_RandomRange":			tooltip	= helpStr[GetLang(), 8];		break;

			case "m_fPrefabSpeed":			tooltip	= helpStr[GetLang(), 9];		break;
			case "m_fPrefabLifeTime":		tooltip	= helpStr[GetLang(),10];		break;
			case "m_AddStartPos":			tooltip	= helpStr[GetLang(),11];		break;

			case "Select Prefab":			tooltip	= helpStr[GetLang(),12];		break;
			case "Clear Prefab":			tooltip	= helpStr[GetLang(),13];		break;
			case "Open Prefab":				tooltip	= helpStr[GetLang(),14];		break;
	...
	public static string GetHsEditor_NcAttachSound(string caption)
	{
		...
		{
			case "m_fDelayTime":		tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fRepeatTime":		tooltip	= helpStr[GetLang(), 2];		break;
			case "m_nRepeatCount":		tooltip	= helpStr[GetLang(), 3];		break;
			case "m_AudioClip":			tooltip	= helpStr[GetLang(), 4];		break;
	...
	public static string GetHsEditor_NcAutoDestruct(string caption)
	{
		...
		switch (caption)
		{
			case "m_fLifeTime":				tooltip	= helpStr[GetLang(), 1];		break;
			case "m_bEndNcCurveAnimation":	tooltip	= helpStr[GetLang(), 2];		break;

			case "m_fSmoothDestroyTime":	tooltip	= helpStr[GetLang(), 3];		break;
			case "m_bDisableEmit":			tooltip	= helpStr[GetLang(), 4];		break;
			case "m_bSmoothHide":			tooltip	= helpStr[GetLang(), 5];		break;

			case "m_CollisionType":			tooltip	= helpStr[GetLang(), 6];		break;
			case "m_CollisionLayer":		tooltip	= helpStr[GetLang(), 7];		break;
			case "m_fCollisionRadius":		tooltip	= helpStr[GetLang(), 8];		break;
			case "m_fDestructPosY":			tooltip	= helpStr[GetLang(), 9];		break;
	...
	public static string GetHsEditor_NcBillboard(string caption)
	{
		...
		switch (caption)
		{
			case "m_bCameraLookAt":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_bFixedObjectUp":		tooltip	= helpStr[GetLang(), 2];		break;
			case "m_bFixedStand":			tooltip	= helpStr[GetLang(), 3];		break;
			case "m_FrontAxis":				tooltip	= helpStr[GetLang(), 4];		break;

			case "m_RatationAxis":			tooltip	= helpStr[GetLang(), 5];		break;
			case "m_RndAxis":				tooltip	= helpStr[GetLang(), 6];		break;
			case "m_fRotationValue":		tooltip	= helpStr[GetLang(), 7];		break;
	...
	public static string GetHsEditor_NcDelayActive(string caption)
	{
		...
		{
			case "m_fDelayTime":			tooltip	= helpStr[GetLang(), 1];	break;
			case "m_bActiveRecursively":	tooltip	= helpStr[GetLang(), 2];	break;
			case "m_fAliveTime":			tooltip	= helpStr[GetLang(), 3];	break;
			case "m_fParentDelayTime":		tooltip	= helpStr[GetLang(), 4];	break;
	...
	public static string GetHsEditor_NcDetachParent(string caption)
	{
		string[,]	helpStr = {
								{
									...
									,"- true: If ParentGameObject becomes Deactive, the timer of automatic removal starts; after m_fSmoothDestroyTime elapses, it is removed." + "\n" +
									 "- false: If gameObject becomes activeSelf, the timer of automatic removal starts; after m_fSmoothDestroyTime, it is removed."
		...
		switch (caption)
		{
			case "m_bFollowParentTransform":	tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fSmoothDestroyTime":		tooltip	= helpStr[GetLang(), 2];		break;
			case "m_bParentHideToStartDestroy":	tooltip	= helpStr[GetLang(), 3];		break;
			case "m_bDisableEmit":				tooltip	= helpStr[GetLang(), 4];		break;
			case "m_bSmoothHide":				tooltip	= helpStr[GetLang(), 5];		break;
	...
	public static string GetHsEditor_NcDuplicator(string caption)
	{
		...
		switch (caption)
		{
			case "m_fDuplicateTime":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_nDuplicateCount":			tooltip	= helpStr[GetLang(), 2];		break;
			case "m_fDuplicateLifeTime":		tooltip	= helpStr[GetLang(), 3];		break;
			case "m_AccumStartRot":				tooltip	= helpStr[GetLang(), 4];		break;
			case "m_RandomRange":				tooltip	= helpStr[GetLang(), 5];		break;
			case "m_AddStartPos":				tooltip	= helpStr[GetLang(), 6];		break;
	...
	public static string GetHsEditor_NcParticleSystem(string caption)
	{
		...
		switch (caption)
		{
			case "m_fStartDelayTime":					tooltip	= helpStr[GetLang(), 1];		break;
			case "m_bBurst":							tooltip	= helpStr[GetLang(), 2];		break;
			case "m_fBurstRepeatTime":					tooltip	= helpStr[GetLang(), 3];		break;
			case "m_nBurstRepeatCount":					tooltip	= helpStr[GetLang(), 4];		break;
			case "m_fBurstEmissionCount":				tooltip	= helpStr[GetLang(), 5];		break;
			case "m_fEmitTime":							tooltip	= helpStr[GetLang(), 6];		break;
			case "m_fSleepTime":						tooltip	= helpStr[GetLang(), 7];		break;

			case "m_bScaleWithTransform":				tooltip	= helpStr[GetLang(), 8];		break;
			case "m_fStartSizeRate":					tooltip	= helpStr[GetLang(), 9];		break;
			case "m_fStartLifeTimeRate":				tooltip	= helpStr[GetLang(), 10];		break;
			case "m_fStartEmissionRate":				tooltip	= helpStr[GetLang(), 11];		break;
			case "m_fStartSpeedRate":					tooltip	= helpStr[GetLang(), 12];		break;
			case "m_fRenderLengthRate":					tooltip	= helpStr[GetLang(), 13];		break;

			case "m_fLegacyMinMeshNormalVelocity":		tooltip	= helpStr[GetLang(), 14];		break;
			case "m_fLegacyMaxMeshNormalVelocity":		tooltip	= helpStr[GetLang(), 15];		break;

			case "m_fShurikenSpeedRate":				tooltip	= helpStr[GetLang(), 16];		break;
			case "m_bWorldSpace":						tooltip	= helpStr[GetLang(), 17];		break;

			case "m_ParticleDestruct":					tooltip	= helpStr[GetLang(), 18];		break;
			case "m_CollisionLayer":					tooltip	= helpStr[GetLang(), 19];		break;
			case "m_fCollisionRadius":					tooltip	= helpStr[GetLang(), 20];		break;
			case "m_fDestructPosY":						tooltip	= helpStr[GetLang(), 21];		break;

			case "m_AttachPrefab":						tooltip	= helpStr[GetLang(), 22];		break;
			case "m_fPrefabScale":						tooltip	= helpStr[GetLang(), 23];		break;
			case "m_fPrefabSpeed":						tooltip	= helpStr[GetLang(), 24];		break;
			case "m_fPrefabLifeTime":					tooltip	= helpStr[GetLang(), 25];		break;

			case "Select Prefab":						tooltip	= helpStr[GetLang(), 26];		break;
			case "Clear Prefab":						tooltip	= helpStr[GetLang(), 27];		break;
			case "Open Prefab":							tooltip	= helpStr[GetLang(), 28];		break;
	...
	public static string GetHsEditor_NcRotation(string caption)
	{
		...
		switch (caption)
		{
			case "m_bWorldSpace":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_vRotationValue":		tooltip	= helpStr[GetLang(), 2];		break;
...
//  		return caption + "\n" + tooltip;
	...
	public static string GetHsEditor_NcSpriteTexture(string caption)
	{
		...
		switch (caption)
		{
			case "m_NcSpriteFactoryPrefab":		tooltip	= helpStr[GetLang(), 1];		break;
			case "m_nSpriteListIndex":			tooltip	= helpStr[GetLang(), 2];		break;
			case "Select SpriteFactory":		tooltip	= helpStr[GetLang(), 3];		break;
			case "Clear SpriteFactory":			tooltip	= helpStr[GetLang(), 4];		break;
	...
	public static string GetHsEditor_NcTilingTexture(string caption)
	{
		...
		switch (caption)
		{
			case "m_fTilingX":			tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fTilingY":			tooltip	= helpStr[GetLang(), 2];		break;
			case "m_fOffsetX":			tooltip	= helpStr[GetLang(), 3];		break;
			case "m_fOffsetY":			tooltip	= helpStr[GetLang(), 4];		break;
			case "m_bFixedTileSize":	tooltip	= helpStr[GetLang(), 5];		break;
	...
	public static string GetHsEditor_NcUvAnimation(string caption)
	{
		...
		switch (caption)
		{
			case "m_fScrollSpeedX":		tooltip	= helpStr[GetLang(), 1];		break;
			case "m_fScrollSpeedY":		tooltip	= helpStr[GetLang(), 2];		break;
			case "m_fTilingX":			tooltip	= helpStr[GetLang(), 3];		break;
			case "m_fTilingY":			tooltip	= helpStr[GetLang(), 4];		break;
			case "m_fOffsetX":			tooltip	= helpStr[GetLang(), 5];		break;
			case "m_fOffsetY":			tooltip	= helpStr[GetLang(), 6];		break;
			case "m_bFixedTileSize":	tooltip	= helpStr[GetLang(), 7];		break;
			case "m_bRepeat":			tooltip	= helpStr[GetLang(), 8];		break;
