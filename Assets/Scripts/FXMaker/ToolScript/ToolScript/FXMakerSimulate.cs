...
[AddComponentMenu("FXMaker/ToolScript/FXMakerSimulate")]
public class FXMakerSimulate : MonoBehaviour
{
	// Attribute ------------------------------------------------------------------------
	public	enum	MODE_TYPE			{NONE, MOVE, ARC, ROTATE, TORNADO, SCALE};

	public			MODE_TYPE			m_Mode;
	public			NgEnum.AXIS			m_nAxis;
...
// 			fHeight = Mathf.Max(m_Curve.keys[n].value);
	...
	public void SimulateScale(NgEnum.AXIS nTransAxis, float fHalfDist, float fSpeed, bool bRotFront)
	{
		Vector3	pos		= transform.position;
		m_nAxis			= nTransAxis;
		m_StartPos		= pos;
		m_EndPos		= pos;
		m_EndPos[(int)m_nAxis] += fHalfDist*2;
		m_fDist			= Vector3.Distance(m_StartPos, m_EndPos);
