...
[AddComponentMenu("FXMaker/ToolScript/FxmFolderPopup_Prefab")]
	...
	protected override void DrawBottomRect(Rect baseRect)
	{
		...
		if (m_bSaveDialog)
		{
			...
			if (m_SelObjectContent == null)
			{
				...
				{
					Texture2D preview = AssetPreview.GetAssetPreview(m_OldSelectMesh);
...
// 		else m_PrefabObjects	= NgAsset.GetPrefabList(m_LoadDirectory, false, true, FXMakerLayout.m_nMaxPrefabListCount, out m_nObjectCount);
		...
		else m_ObjectNodes	= NgAsset.GetPrefabList(m_LoadDirectory, true, true, FXMakerLayout.m_nMaxPrefabListCount, true, out m_nObjectCount);
