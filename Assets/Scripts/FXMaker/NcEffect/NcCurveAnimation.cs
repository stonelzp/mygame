...
[AddComponentMenu("FXMaker/NcEffect/NcCurveAnimation")]
				...
				case NcInfoCurve.APPLY_TYPE.COLOR:
					{
						if (curveInfo.m_bRecursively)
						{
							...
							else
							{
								...
								if(projector != null)
								{
									m_ColorName = Ng_GetMaterialColorName(projector.material);
									if (m_ColorName != null)
										curveInfo.m_OriginalValue = projector.material.GetColor(m_ColorName);
									curveInfo.m_BeforeValue		= Vector4.zero;
								}
							}
						}
							...
							if (m_ColorName != null)
							{
								if (m_MainMaterial == null)
								{
									...
									else
									{
										Projector projector = GetComponent<Projector>();
										if(projector != null)
											m_MainMaterial = projector.material;
									}
								}
