...
static public class NGUITools
{
	...
	static public BoxCollider AddWidgetCollider (GameObject go, bool bIncludeInactive)
	{
		if (go != null)
		{
...
//			return box;
			...
			float fd = 0;
			UIWidget[] widgets = go.GetComponentsInChildren<UIWidget>(bIncludeInactive);
			for (int i = 0, imax = widgets.Length; i < imax; ++i)
			{
				float fn = widgets[i].layer * 5 + widgets[i].depth * 0.1f;
				//float fn = widgets[i].depth * 0.1f;
				if(widgets[i].gameObject != go) fn -= widgets[i].transform.localPosition.z;
				fd = Mathf.Max(fd, fn);
			}

			Bounds b = NGUIMath.CalculateRelativeWidgetBounds(go.transform, bIncludeInactive);
			box.isTrigger = true;
			box.center = new Vector3(b.center.x, b.center.y, -fd);
			box.size = new Vector3(b.size.x, b.size.y, 0f);
			return box;
		}
		return null;
