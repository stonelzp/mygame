...
public class UIPanel : MonoBehaviour
{
	...
	//添加一个删除控件队列
	List<UIWidget>	mDelWidgetsList = new List<UIWidget>();
	...
	// collider oper
	static BetterList<string> mCollinders = new BetterList<string>();
	
	public void AddDelWidget(UIWidget widget)
	{
		widget.gameObject.SetActive(false);
		mDelWidgetsList.Add(widget);
	}
	
	public static void addColliderPanel(string name)
	{
		mCollinders.Add(name);
	}
	
	public static void rmColliderPanel(string name)
	{
		mCollinders.Remove(name);
	}
...
//#else
			...
			GameObject go = new GameObject("__UIDrawCall [" + name + "__" + mat.name + "]");
...
//#endif
		...
		if (mCheckVisibility || transformsChanged || mRebuildAll)
		{
			...
			for (int i = 0, imax = mChildren.Count; i < imax; ++i)
			{
				...
				if (pc.widget != null)
				{
					...
					if ( mCollinders.Contains(transform.name) )
					{
						BoxCollider bc = pc.widget.GetComponent<BoxCollider>();
						UILabel label = pc.widget.GetComponent<UILabel>();
						if ( 1 == visibleFlag && null == bc && label != null )
							NGUITools.AddWidgetCollider(pc.widget.gameObject);
						if ( 0 == visibleFlag && null != bc && label != null )
							Destroy(bc);
					}
	...
	void UpdateWidgets ()
	{
		...
		foreach (KeyValuePair<Transform, UINode> c in mChildren)
		{
			UINode pc = c.Value;
#else
		for (int i = 0, imax = mChildren.Count; i < mChildren.Count; ++i)
		{
			UINode pc = (UINode)mChildren[i];
#endif
		...
		for(int i = 0; i < mDelWidgetsList.Count; i++)
		{
			Destroy(mDelWidgetsList[i].gameObject);
		}
		mDelWidgetsList.Clear();
	}
	...
	public void UpdateDrawcalls ()
	{
		...
		for (int i = 0, imax = mDrawCalls.size; i < imax; ++i)
		{
			...
			Vector3 rootScale	= new Vector3(1,1,1);
			if(UIRoot.list.Count > 0)
				rootScale	= UIRoot.list[0].transform.localScale;
			dt.position = new Vector3(t.position.x,t.position.y,t.position.z -5 * rootScale.z* dc.layer);
	...
	void LateUpdate ()
	{
		...
		//重新对所有修改的控件设置他们的层级，层级不一致会导致在mWidgets中的顺序不一致，顺序决定了
		//先后绘制关系，决定了最终显示效果
		for (int i = 0, imax = mChanged.size; i < imax; ++i) AdjustLayer(mChanged.buffer[i]);
	...
	public void UpdateLayer(Material mat,int layer)
	{
		for (int i = 0; i < mDrawCalls.size; ++i)
		{
			UIDrawCall dc = mDrawCalls[i];
			if(dc.material	== mat && dc.layer != layer)
			{
				dc.layer	= layer;
				MarkMaterialAsChanged(dc.material, false);
			}
		...
		for (int i = 0; i < mWidgets.size; ++i)
		{
			if(mWidgets[i].material == mat && mWidgets[i].layer != layer)
			{
				mWidgets[i].layer = layer;
				mWidgets[i].MarkAsChangedLite();
			}
		}
	}
	...
	//all widget z set 0
	public void SetAllWidgetToZero()
	{
//		for (int i = 0; i < mWidgets.size; ++i)
//		{			
//			mWidgets[i].transform.localPosition	= new Vector3(mWidgets[i].transform.localPosition.x,mWidgets[i].transform.localPosition.y,0.0f);
//		}
							
		Transform[] trans = GetComponentsInChildren<Transform>(true);
		for(int i=0; i<trans.Length; i++) 
			trans[i].localPosition	= new Vector3(trans[i].localPosition.x,trans[i].localPosition.y,0.0f);
