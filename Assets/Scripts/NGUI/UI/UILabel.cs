...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] string mCommonTips = "";
	[HideInInspector][SerializeField] bool mVertical = false;
	...
	//添加	
	RenderStr mRenderStr = new RenderStr();
	...
	public delegate void OnMouseHover(bool hover);
	public OnMouseHover onMouseHover;
	...
	protected override void OnStart ()
	{
		...
		mTextOp.Init(mRenderStr,this);
		
		//为显示公共TIPS,判断是否有BoxCollider
		if(!string.IsNullOrEmpty(this.CommonTips))
		{
	...
	void ProcessText ()
	{
		...
		if ( ShowVertical )
		{
			mProcessedText = appendWrap(mText);
		}
		else
		{
...
//	}
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		renderString.Draw(verts,uvs,cols);
	}
	
	//添加RenderString 成员
	public RenderStr renderString
	{
		get
		{
			ParseStringMgr.SP.mGo		= gameObject;
			ParseStringMgr.SP.Encoding	= mEncoding;
			mRenderStr.StrFont			= mFont;			
			mRenderStr.LineWidth		= lineWidth;
			mRenderStr.LabelScale		= transform.localScale.x;
			mRenderStr.Label			= this;
			
			//设置对齐方式
			if (pivot == Pivot.Left || pivot == Pivot.TopLeft || pivot == Pivot.BottomLeft)
			{
				mRenderStr.StrAlignment	= UIFont.Alignment.Left;
			}
			else if (pivot == Pivot.Right || pivot == Pivot.TopRight || pivot == Pivot.BottomRight)
			{
				mRenderStr.StrAlignment = UIFont.Alignment.Right;
			}
			else
			{
				mRenderStr.StrAlignment = UIFont.Alignment.Center;
			}
			
			ParseStringMgr.SP.Parse(processedText,mFont,color,mRenderStr,effectStyle,effectColor,effectDistance);
			return mRenderStr;
		}
	}
	
	private XUITextOperation mTextOp = new XUITextOperation();
	...
	void OnTooltip(bool state)
	{
		...
		if (state) 
		{
			if (!string.IsNullOrEmpty(CommonTips)) 
			{
	...
	private string appendWrap(string input)
	{
		...
		for( int i = 0; i < input.Length; i++ )
		{
			...
			if ( ch == '[' )
			{
				...
				if ( pos > 0 )
				{
					sb.Append(input.Substring(i, pos - i + 1));
					i = i + pos;
					continue;
				}
			}
			
			if ( numberBefore && (ch <'0' || ch >'9') )
				sb.Append("\n");
