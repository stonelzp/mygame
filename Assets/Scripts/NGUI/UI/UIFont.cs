...
public class UIFont : MonoBehaviour
{
	...
	public Vector2 CalculatePrintedSize (string text, bool encoding, SymbolStyle symbolStyle)
	{
...
#endif
		{
			...
			for (int i = 0; i < length; ++i)
			{
				...
				if (encoding && c == '[')
				{
					int endPos = ParseStringMgr.SP.FindControlStringPos(text,i);
					if(endPos != -1)
					{
						i = endPos;
						continue;
	...
	public string WrapText (string text, float maxWidth, int maxLineCount, bool encoding, SymbolStyle symbolStyle)
	{
		...
		//先做文本替换
		text = ParseStringMgr.SP.ReplaceText(text);
...
//			}
			if (encoding && ch == '[')
			{
				int endPos = ParseStringMgr.SP.FindControlStringPos(text,offset);
				if(endPos != -1)
				{
					int pos1 = text.IndexOf(']',offset);
					string sub = text.Substring(offset + 1, pos1 - offset - 1);
					int cwidth = 0;
					int ret = calComponentWrapText(sub, remainingWidth, ref cwidth);
					if ( 1 == ret )
					{
	...
	public void Align (BetterList<Vector3> verts, int indexOffset, Alignment alignment, int x, int lineWidth,float LabelScale)
	{
	...
	public void Align (BetterList<Vector3> verts, int indexOffset, Alignment alignment, int x, int lineWidth)
	{
		if (alignment != Alignment.Left)
		{
			int fs = size;
	...
	int calComponentWrapText(string componentStr, int remainWidth, ref int componentWidth)
	{
		string[] splitStr1 = componentStr.Split('=');
		if ( splitStr1.Length < 2 || splitStr1[0] != "sprite" )
			return -1;
		string[] splitStr2 = splitStr1[1].Split('.');
		if ( splitStr2.Length < 4 )
			return -1;
		
		componentWidth = int.Parse(splitStr2[2]);
		if ( remainWidth - componentWidth > 0 )
			return 0;
		else
			return 1;
	}
	...
	{
		Vector2 scale = size > 0 ? new Vector2(1f / size, 1f / size) : Vector2.one;
		return scale;
	}
	
	//新增字体解析的绘制方法
	public void PrintParseStr(Vector2 startPos,string text, Color32 color, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		if (text != null)
		{
			if (!isValid)
...
//				}
				...
				if (!dynamic)
				{
					// See if there is a symbol matching this text
					//BMSymbol symbol = useSymbols ? MatchSymbol(text, i, textLength) : null;
					BMSymbol symbol = MatchSymbol(text, i, textLength);
					if (symbol == null)
					{
						BMGlyph glyph = mFont.GetGlyph(c);
						if (glyph == null) continue;
						...
						v0.x =  startPos.x + invSize.x * (x + glyph.offsetX);
						v0.y = -startPos.y -invSize.y * (y + glyph.offsetY);
				...
				else
				{
					...
					v0.x =  startPos.x + invSize.x * (x + mChar.vert.xMin);
					v0.y = -startPos.y -invSize.y * (y - mChar.vert.yMax + mDynamicFontOffset);
...
//			}
	...
	{
		float scale = (size > 0) ? 1f / size : 1f;
		int lineHeight = (size + mSpacingY);
		return scale * lineHeight;
	}
	
	public bool GetCharUV(char c,ref Vector2 uv0,ref Vector2 uv1,ref bool isFlipped)
	{
		uv0 = Vector2.zero;
		uv1 = Vector2.zero;
		...
		{			
			...
			mDynamicFont.RequestCharactersInTexture(temp, mDynamicFontSize, mDynamicFontStyle);
			if (!mDynamicFont.GetCharacterInfo(c, out mChar, mDynamicFontSize, mDynamicFontStyle))
				return false;
	...
	public int GetCharAtPixel (string text, int startIndex,float dist,bool encoding, SymbolStyle symbolStyle)
	{
		if (mReplacement != null) return mReplacement.GetCharAtPixel(text,startIndex,dist,encoding,symbolStyle);
