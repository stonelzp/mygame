...
{
	public delegate void OnUnPress();
	public static OnUnPress UnPressDelegate;
	...
	void AdjustCameraSize()
	{
		...
		{
			float size 		= cachedCamera.orthographicSize;
			//float height 	= Mathf.RoundToInt(cachedCamera.pixelHeight / 2);
			float height 	= cachedCamera.pixelHeight / 2;
			if(size != height)
				cachedCamera.orthographicSize	= height ;
		}
	}
	...
	public void ProcessMouse ()
	{
		...
		if (useMouse && mHover != null && mHover != mMouse[0].current)
		{
		...
		if (useMouse &&  mHover != mMouse[0].current)
		...
		//add mouse move event zbz
		if(useMouse && mMouse[0].current != null)
		{
			Notify(mMouse[0].current, "OnMouseMove",null);
		}
	}
		...
		if (unpressed)
		{
			...
			if (currentTouch.pressed != null)
			{
				...
				//{
					...
					if(currentTouch.totalDelta.magnitude > drag)
					{
						Notify(currentTouch.current, "OnDrop", currentTouch.dragged);
					
						if(!currentTouch.DropIsDeal)
							Notify(currentTouch.dragged, "OnDrop",currentTouch.dragged);
					
						currentTouch.DropIsDeal	= false;
					}
			...
			if(UnPressDelegate != null)
				UnPressDelegate();
		}
		
		if(currentTouch.pressed != null)
			Notify(currentTouch.pressed, "OnPressing", null);
	}
