...
{
	public delegate char Validator (string currentText, char nextChar,int maxValue);
	...
	void Update()
	{
		if (mKeyboard != null)
		{
			...
			if (mText != text)
			{
				...
				{
					char ch = text[i];
					if (validator != null) ch = validator(mText, ch,MaxValidValue);
					if (ch != 0) mText += ch;
				}
	...
	void Append (string input)
	{
		for (int i = 0, imax = input.Length; i < imax; ++i)
		{
			...
			else if (c == '\r' || c == '\n')
			{
				...
				if (validator != null) c = validator(mText, c,MaxValidValue);
