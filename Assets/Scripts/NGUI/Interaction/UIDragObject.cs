...
public class UIDragObject : IgnoreTimeScale
{
	...
	void OnDrag (Vector2 delta)
	{
		if (enabled && NGUITools.GetActive(gameObject) && target != null)
		{
			...
			//对超出屏幕范围的不做处理
			if(UICamera.currentTouch.pos.x < 0)
				UICamera.currentTouch.pos	= new Vector2(0,UICamera.currentTouch.pos.y);
			if(UICamera.currentTouch.pos.x > Screen.width)
				UICamera.currentTouch.pos	= new Vector2(Screen.width,UICamera.currentTouch.pos.y);
			
			if(UICamera.currentTouch.pos.y < 0)
				UICamera.currentTouch.pos	= new Vector2(UICamera.currentTouch.pos.x,0);
			if(UICamera.currentTouch.pos.y > Screen.height)
				UICamera.currentTouch.pos	= new Vector2(UICamera.currentTouch.pos.x,Screen.height);
