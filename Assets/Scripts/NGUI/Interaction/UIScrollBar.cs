...
public class UIScrollBar : MonoBehaviour
{
	...
	public delegate void OnSizeMax(float x, float y);
	
	public delegate void OnScrollBarShowState(bool state);
	
	public UIImageButton	BtnDown;
	public UIImageButton	BtnUp;
	...
	[HideInInspector][SerializeField] bool mTop = false;
	...
	public OnSizeMax onSizeMax;
	
	public OnScrollBarShowState onScrollBarShowState;
	...
	public bool Top { get { return mTop; } set { if (mTop != value) { mTop = value;} } }
	...
	public float barSize
	{
		...
		set
		{
			...
			if (mSize != val)
			{
				...
				mIsDirty = true;
				if (onChange != null) onChange(this);
				if ( ( 1.0f == mSize || 0f == mSize ) && null != onSizeMax )
					onSizeMax(0, (Top ? 0f : 1f));
			}
		}
	}
	...
	public float alpha
	{
		...
		set
		{
			if (mFG != null)
			{
				...
				{
					NGUITools.SetActiveSelf(BtnUp.gameObject, (mFG.alpha > 0.001f));
					NGUITools.SetActiveSelf(BtnDown.gameObject, (mFG.alpha > 0.001f));
				}
			}
			...
			if(onScrollBarShowState != null)
			{
				onScrollBarShowState((mBG.alpha > 0.001f));
			}
		}
	}
