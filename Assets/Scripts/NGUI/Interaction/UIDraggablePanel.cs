...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	void Awake ()
	{
		...
		if ( verticalScrollBar != null )
			verticalScrollBar.onSizeMax = ResizeMFormPos;
	}
	
	private void ResizeMFormPos(float x, float y)
	{
	...
	public void SetDragAmount (float x, float y, bool updateScrollbars)
	{
		...
		if (!updateScrollbars)
		{
			...
			mTrans.localPosition = new Vector3(Mathf.RoundToInt(pos.x),Mathf.RoundToInt(pos.y),Mathf.RoundToInt(pos.z));
		}
	...
	void OnVerticalBar (UIScrollBar sb)
	{
		if (!mIgnoreCallbacks)
		{
			float x = (horizontalScrollBar != null) ? horizontalScrollBar.scrollValue : 0f;
			float y = (verticalScrollBar != null) ? verticalScrollBar.scrollValue : 0f;
			if ( verticalScrollBar.barSize == 0f || verticalScrollBar.barSize == 1f )
				y = (verticalScrollBar.Top ? 0f: 1f);
			SetDragAmount(x, y, false);
		}
	}
