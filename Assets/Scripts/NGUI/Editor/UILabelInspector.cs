...
public class UILabelInspector : UIWidgetInspector
{
	...
	override protected bool DrawProperties ()
	{
		...
		if (mLabel.font != null)
		{
			...
			string CommonTips = (string)EditorGUILayout.TextField("CommonTips", mLabel.CommonTips);
			if(CommonTips != mLabel.CommonTips)
			{	
				mLabel.CommonTips = CommonTips;
			}
			...
			GUILayout.BeginHorizontal();
			bool vertical = EditorGUILayout.Toggle("ShowVertical", mLabel.ShowVertical, GUILayout.Width(100f));
			GUILayout.Label("show text on vertical");
			GUILayout.EndHorizontal();
			if (vertical != mLabel.ShowVertical) { RegisterUndo(); mLabel.ShowVertical = vertical; }
