...
public class UIWidgetInspector : Editor
{
	...
	static UIWidget[] Raycast (UIWidget widget, Vector2 mousePos)
	{
		GameObject root = widget.gameObject;
		UIPanel panel = NGUITools.FindInParents<UIPanel>(root);
		return (panel != null) ? NGUIEditorTools.Raycast(panel, mousePos) : new UIWidget[] {};
	}
	...
	void DrawLayer()
	{
		...
		GUILayout.BeginHorizontal();
		{
			...
			int layer = mWidget.layer;
			if (GUILayout.Button("Back", GUILayout.Width(60f))) --layer;
			layer = EditorGUILayout.IntField(layer);
			if (GUILayout.Button("Forward", GUILayout.Width(60f))) ++layer;

			if (mWidget.layer != layer)
			{
				NGUIEditorTools.RegisterUndo("Layer Change", mWidget);
				mWidget.layer = layer;	
				mDepthCheck = true;
			}
		}
