...
public class UISpriteInspector : UIWidgetInspector
{
	...
	override protected bool DrawProperties ()
	{
		...
		GUILayout.BeginHorizontal();
		{
			GUILayout.Label("CommandTips");
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		{
			string CommonTips = GUILayout.TextArea(mSprite.CommonTips,GUILayout.Height(50));
			if(CommonTips != mSprite.CommonTips)
			{	
				mSprite.CommonTips = CommonTips;
			}
		}
		GUILayout.EndHorizontal();
		return true;
